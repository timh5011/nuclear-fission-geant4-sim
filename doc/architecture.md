...
### Delta-electron filtering

Delta electrons (knock-on e- ejected from atomic shells when a charged particle ionizes the medium it's traversing) are dropped at the source in `MyTrackingAction::PreUserTrackingAction`, before the track is ever recorded in `truth-record.csv` or counted in any `n_chain_*` field. The same early return that skips optical photons skips deltas. The motivation is identical: deltas are an EM byproduct of charged-particle transport through matter, not a step in the nuclear decay chain. Their kinetic energy is already accounted for as part of the parent's KE budget, so admitting them into the truth record double-counts that energy. Any deposition they later make in a scintillator surfaces downstream as scintillation light — i.e. they show up in `hits.csv` exactly as scintillation-stage observables, the same handling as optical photons.

The filter is `pdg == 11 && creator_process ends in "Ioni"`. Both conditions are necessary; neither is sufficient on its own.

- **PDG 11 (`e-`).** Geant4's δ-ray production lives in the *ionization* `G4VEmProcess` family — `G4eIonisation`, `G4ionIonisation`, `G4hIonisation`, `G4MuIonisation` — and the secondary it spawns is always an electron, never any other species. Restricting on `pdg == 11` therefore costs nothing for filtering deltas and protects every other particle type the ionization processes leave alone (the parent track itself, brems photons from `eBrem`, etc.).
- **Creator-process suffix `Ioni`.** The Geant4 process-name convention is that the four ionization processes above all register their `G4VProcess::GetProcessName()` as a string ending in `Ioni`. Matching on that suffix catches all four in one check, and won't be confused with any other electron-producing process — `G4RadioactiveDecay` registers as `RadioactiveDecay`, β decays of fragments register their daughter electron's `GetCreatorProcess()->GetProcessName()` as `RadioactiveDecay`, conversion electrons from γ→e+e- pair production register as `conv`, photoelectrons as `phot`, Compton electrons as `compt`. None of those end in `Ioni`, so none are filtered.

The asymmetry is the point: the filter is *creator-process-based*, not *species-based*. A β⁻ from a fission-fragment decay has `pdg == 11` too, and a naive "drop all electrons" filter would erase the β⁻ spectrum that is the whole reason the truth record exists. But its creator process is `RadioactiveDecay`, not `eIoni`/`ionIoni`, so it survives. Likewise a primary atomic-shell electron from a γ→e+e- conversion (`conv`), a photoelectron (`phot`), or a Compton-scattered atomic electron (`compt`) — these are all rare in the fission stage but if they occur they survive the filter, which is correct: they are not delta rays, they are the primary product of a *different* QED process whose kinetic energy is *not* already counted elsewhere in the chain.

What this filter does *not* claim to do: it is not a general "drop all EM secondaries" filter. The plot scripts apply the broader `creator_process ∈ {primary, nFissionHP, RadioactiveDecay}` filter at plot time to drop `eBrem` photons, `phot`/`compt` electrons, etc. that the C++ filter intentionally lets through. Two layers, two scopes — the C++ filter handles the one EM byproduct that would otherwise dominate the file by row count (delta electrons from heavy-ion ionization of the foil are produced in the tens of thousands per fission); the plot-time filter handles everything else.
...
│   ├── plot_decay_schemes.py         per-event NNDC/ENSDF nuclear decay scheme
│   │                                 (TikZ via pdflatex; one PDF per event)
│   ├── decay_scheme_template.tex.j2  Jinja2-templated TikZ document body for
│   │                                 plot_decay_schemes.py
│   ├── plot_decay_trees.py           per-event nuclear decay tree
│   │                                 (graphviz LR; one PDF per event;
│   │                                 reuses chain-shaping helpers from
│   │                                 plot_decay_schemes.py)
│   └── requirements.txt              pandas, numpy, matplotlib, jinja2, graphviz
│
...
`plot_decay_schemes.py` additionally requires **`pdflatex` on `$PATH`** (TeX Live or MacTeX). It computes the diagram data structures in Python, fills the Jinja2 TikZ template, writes a `.tex` file in a temp directory, and shells out to `pdflatex` to compile the per-event PDF. Without `pdflatex` the script emits a clear error pointing at the install.

`plot_decay_trees.py` additionally requires the **`dot` binary from Graphviz on `$PATH`** (Homebrew `brew install graphviz`, or Anaconda's `graphviz` package, or a pip-installed system `graphviz`). The Python `graphviz` library shells out to `dot` to render the per-event PDF; the script checks `shutil.which("dot")` at startup and emits a clear error if missing. `plot_energy_spectra.py` and `plot_event_multiplicities.py` have no system dependencies beyond the Python packages in `requirements.txt`.
...
```
plots/<ts>_plots/
├── energy-spectra/         6 PDFs from plot_energy_spectra.py — one per category
│                           (neutron / gamma / beta / alpha / ion / other).
├── multiplicities/         8 PDFs from plot_event_multiplicities.py — prompt and
│                           chain n/γ, plus chain-only β / α / ion / other.
├── decay-schemes/          N single-page PDFs from plot_decay_schemes.py — one
│                           per fission event. NNDC/ENSDF-style nuclear decay
│                           scheme covering both fragment chains in one
│                           diagram: horizontal level lines per nuclide,
│                           diagonal arrows for cross-Z β / α transitions,
│                           vertical dashed arrows for in-column γ cascades.
└── decay-trees/            N single-page PDFs from plot_decay_trees.py — one
                            per fission event. graphviz LR layout: each chain
                            entry is a record-shaped node carrying the
                            nuclide, cumulative Q, creator process, and
                            outgoing-decay summary; consecutive nuclides are
                            connected by bare arrows. Layout is solver-driven
                            (`dot`) — no overlap by construction. Companion
                            view to decay-schemes/: the scheme keeps the
                            quantitative energy-axis semantic; the tree gives
                            a clean topological view that scales to dense
                            low-Q chain tails.
```
...
- **Delta electrons are filtered out at the source.** `MyTrackingAction::PreUserTrackingAction` drops any `e-` whose creator process ends in `Ioni` (`eIoni`, `ionIoni`, `hIoni`, `muIoni`) before either the truth row or the chain-counter increment fires. They are an EM byproduct of charged-particle transport, not part of the nuclear decay chain — their KE is already part of the parent's KE budget, and any energy they later deposit in a scintillator surfaces downstream as scintillation light, the same way optical photons do. `plot_energy_spectra.py` and `plot_decay_schemes.py` additionally apply `creator_process ∈ {primary, nFissionHP, RadioactiveDecay}` to drop other EM secondaries (`phot`, `compt`, `eBrem`) at plot time.
...
- **Per-event decay-scheme PDF.** `plot_decay_schemes.py` writes one single-page PDF per fission event into `plots/<ts>_plots/decay-schemes/`. Each diagram is a TikZ-rendered NNDC/ENSDF-style nuclear decay scheme covering **both** fragment chains on the same axes. The rendering pipeline is unusual for a Python analysis script: Python computes a `(columns, levels, transitions)` data structure from the truth-record, fills `decay_scheme_template.tex.j2` via Jinja2, writes the resulting `.tex` to a `tempfile.TemporaryDirectory`, and shells out to `pdflatex -interaction=nonstopmode -halt-on-error` to produce the PDF. The trade-off is one system dependency (`pdflatex`) for publication-quality output — Computer Modern math, native Greek / superscripts / Stealth arrowheads — that hand-rolled matplotlib primitives can't reach. Per-event render cost is ~1–2 s; for runs in the tens of events this is fine.
  - **Diagram convention.** *x-axis* = one column per (Z, A) nuclide encountered across both chains, sorted by Z. The two fragments share columns when they happen to land on the same nuclide (rare in U-235 fission since light and heavy peaks are well separated). Column header above the topmost level shows the nuclide as `${}^{A}\mathrm{Sym}$`. *y-axis* = cumulative Q released since the fragment's birth, increasing downward, with each level annotated by its precise cumulative-Q value to the right. *Horizontal segments* are levels; *diagonal red/blue/orange arrows* are β⁻ / β⁺ / α transitions across columns; *vertical dashed green arrows* are γ de-excitation cascades inside a column.
  - **Chain pre-processing.** Three cleanup passes run between `walk_chain` and the rendered TikZ; all are tunable at module top of `plot_decay_schemes.py`:
    1. **`filter_chain_emissions` — drop emission entries with KE < `EMISSION_KE_FLOOR_MEV` (5 keV)**. Geant4 emits internal-conversion and Auger electrons as `RadioactiveDecay` children of every β decay step; without this filter the per-arrow label fills up with `β⁻: 0.000` rows that carry no decay-scheme-scale information.
    2. **`coalesce_isomers` — merge consecutive same-(Z, A) chain entries**. Geant4's IT cascades show up in the truth-record as `Pr150 → Pr150 → Pr150 → Nd150` (one chain entry per emitted γ); the coalesce pass folds the leading γ-only steps into the first non-γ-only step that follows so the diagram has at most one row per nuclide. After coalesce, no chain contains repeated consecutive same-(Z, A) ions. Terminal nuclides (no further decay in the sim window) are *not* merged into; they keep their own level with the "(stable / no decay in sim window)" annotation.
    3. **γ-cascade collapse, in `build_diagram`.** Each decay step's γ photons are merged into a *single* dashed arrow from the populated excited level down to the daughter ground level, labelled `γ × N: E_total MeV` (or `γ: E_γ MeV` when N == 1). The intermediate excited levels that one γ arrow per photon would have required are dropped. Rationale: the cascade is one physical event in Geant4's bookkeeping (N sibling photons under the same `RadioactiveDecay` vertex), and the actionable energy quantity at decay-scheme scale is the total cascade energy — which is also what `E*` on the topmost excited level annotates. Per-γ energies remain available in `truth-record.csv` for any reader who needs them.
  - **Layout.** Within each column, levels are pushed apart to a minimum vertical gap of `MIN_LEVEL_GAP_CM` (0.55 cm) by `_enforce_min_level_gap`. The provisional y-coordinate is `Y_SCALE` (0.70 cm/MeV) × cumulative-Q-in-MeV, then this min-gap pass shifts colliding levels downward. The y-axis is therefore *nominal* (monotonic and roughly proportional to Q), not strictly linear — the precise cumulative-Q value is annotated at the right of each level line, which is the form a reader uses anyway. Cross-column β/α arrow labels sit at `pos=0.32` along the diagonal (closer to the parent column, which is less crowded than the daughter column where the γ cascade lives). γ-arrow labels are shifted 5 mm to the right of their dashed shaft via the `garrowlabel` TikZ style so they don't sit on top of the arrow.
  - **Driver-emission compaction.** A β decay's label combines β⁻ and ν̄_e onto one line — `$\beta^- + \bar{\nu}_e$: 0.508 + 2.893 MeV` — instead of stacking them. The label is hard-capped at `MAX_DRIVER_LINES = 3` lines to defend against pathological cases the 5-keV filter doesn't catch.
- **Per-event decay-tree PDF.** `plot_decay_trees.py` writes a second per-event PDF into `plots/<ts>_plots/decay-trees/` — graphviz `dot` LR layout, one record-shaped node per chain entry, edges connecting consecutive nuclides. **The defining property is "no overlap, by construction"**: `dot` lays out node bounding boxes such that none collide. This is the property the NNDC scheme cannot have because of its quantitative coordinate semantic, and it is why both views coexist — the scheme stays as the energy-axis publication-conventional view, the tree handles dense low-Q chain tails the scheme can't lay out cleanly. The chain-shaping pipeline (`walk_chain` → `filter_chain_emissions` → `coalesce_isomers`) is imported from `plot_decay_schemes.py` so the two scripts share semantics by construction; only the rendering differs.
  - **Pipeline.** `graphviz.Digraph(format='pdf')` populated per chain entry → `g.pipe(format='pdf')` returns the rendered bytes → write directly to `out_pdf`. No tempfile / `g.render()` shell artifact. Each fragment chain lives in its own `cluster_fragment_A` / `cluster_fragment_B` subgraph (dashed grey border) so a multi-fragment event reads as two stacked labelled boxes.
  - **Node design (HTML labels).** Each node is a 3-section HTML table: top section (light yellow) = nuclide identity (`²³⁵U` etc., with UTF-8 superscripts); middle section (white) = `cum Q = X.XXX MeV` and `created via: nFissionHP|RadioactiveDecay`; bottom section = decay summary, color-coded by outgoing kind. Bottom-section background and node border match the NNDC arrow palette (β⁻ light coral / red, β⁺ light blue / blue, α light orange / orange, γ-only IT light green / green, terminal light grey). For the bottom section: β/α steps show kind + Q + drivers (combined β⁻+ν̄_e same as the scheme arrow) + the same `γ × N: total MeV` summary line if a γ cascade is present; γ-only IT steps show just the IT header and the cascade summary; terminal nodes show `(stable / no decay in sim window)` instead of a decay summary.
  - **Edges.** Bare black arrows, no labels — the decay information already lives in the parent node's bottom section, so labelling the edge would be redundant. Edge style is `dot`'s default arrowhead.
  - **Why not replace the scheme.** The scheme's y-axis = cumulative Q is genuinely useful: it gives a reader the "how much energy did each step release" intuition at a glance. The tree loses that — node y-position is layout-driven, not quantitative. The trade-off is per-diagram; both are kept and a reader can switch between them.
