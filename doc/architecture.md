...
- **EM transport secondaries are filtered out by default.** `plot_energy_spectra.py` (except the `_with_deltas` variant) and `plot_decay_schemes.py` apply `creator_process ∈ {primary, nFissionHP, RadioactiveDecay}` and drop everything else (`ionIoni`, `eIoni`, `phot`, `compt`, `eBrem`). Without this filter ~97 % of `truth-record.csv` rows are sub-keV delta electrons from heavy-ion ionization inside the foil; their kinetic energy is already part of the parent fragment's KE budget so plotting them adds no independent information. The double-counting argument is developed in `doc/theory.md`.
...
- **Per-event decay-scheme PDF.** `plot_decay_schemes.py` writes one single-page PDF per fission event into `plots/<ts>_plots/decay-schemes/`. Each diagram is a TikZ-rendered NNDC/ENSDF-style nuclear decay scheme covering **both** fragment chains on the same axes. The rendering pipeline is unusual for a Python analysis script: Python computes a `(columns, levels, transitions)` data structure from the truth-record, fills `decay_scheme_template.tex.j2` via Jinja2, writes the resulting `.tex` to a `tempfile.TemporaryDirectory`, and shells out to `pdflatex -interaction=nonstopmode -halt-on-error` to produce the PDF. The trade-off is one system dependency (`pdflatex`) for publication-quality output — Computer Modern math, native Greek / superscripts / Stealth arrowheads — that hand-rolled matplotlib primitives can't reach. Per-event render cost is ~1–2 s; for runs in the tens of events this is fine.
  ...
  - **Layout.** Within each column, levels are pushed apart to a minimum vertical gap of `MIN_LEVEL_GAP_CM` (0.45 cm) by `_enforce_min_level_gap`. This makes the y-axis nominal rather than strictly proportional to cumulative Q — the precise value is annotated at the right of each level line, which is the form a reader uses anyway. Cross-column β/α arrow labels sit at `pos=0.32` along the diagonal (closer to the parent column, which is less crowded than the daughter column where the γ cascade lives). γ-arrow labels are shifted 5 mm to the right of their dashed shaft via the `garrowlabel` TikZ style so they don't sit on top of the arrow.
