...
│   ├── plot_event_energy_budgets.py  per-event 3-page PDF (Sankey + partition bar + decay scheme)
...
└── energy-budgets/         N 3-page PDFs from plot_event_energy_budgets.py
                            (one per fission event). Each PDF contains, in
                            order: a chronological total-energy Sankey
                            (page 1, ribbon width = KE + mc², linear), a
                            textbook ENDF-style energy-partition bar chart
                            (page 2), and a NNDC-style nuclear decay scheme
                            with one staircase per fragment (page 3).
```
...
- **Energy-budget PDF, three pages.** `plot_event_energy_budgets.py` writes one PDF per fission event with three diagrams in order. Each page is rendered by its native library (Plotly + Kaleido for the Sankey, matplotlib for the bar and decay scheme) into a single-page PDF in a `tempfile.TemporaryDirectory`, then concatenated into the final per-event PDF using `pypdf.PdfWriter`. The three pages are independent diagrams, not subplots — they share only the underlying truth-record slice for that event and the same nuclear-process filter.
  - **Page 1 — Total-energy Sankey.** Chronological parent → child topology of the decay tree. Source node is the fission step (depth 0); subsequent depths are assigned by BFS along the parent_track_id chain. Each node is bucketed by `(particle, depth)` — both ions and non-ions get a depth tag. Node x-coordinates are computed from depth and passed to Plotly with `arrangement="perpendicular"` so the d3-sankey solver picks `node.y` to minimize edge crossings while honoring the depth columns. **Ribbon width = KE + mc²** (linear scale): the source node and ion ribbons are dominated by ~125-127 GeV of fragment rest mass, while lepton/photon ribbons are ~MeV-scale and consequently hairline-thin. This is intentional — the user explicitly chose the relativistic-total-energy convention to see the rest-mass dominance directly. Strict node-level energy conservation holds because total relativistic energy is additive at every decay vertex (it's literal `E_in = E_out`). **Why ions also carry the depth tag (not just non-ions):** Geant4's RadioactiveDecay process emits the same nuclide name for excited isomers and their ground-state daughters when the de-excitation is by internal γ emission — e.g. Y94* → Y94 + γ produces two `truth-record` rows both named `"Y94"` at adjacent depths. Without the depth tag in the bucket key, those two collapse to one node and the de-excitation step renders as a self-loop. Tagging by depth gives them distinct nodes at distinct columns and the de-excitation renders as a normal cross-column ribbon.
  - **Page 2 — Energy-partition bar chart.** Static horizontal stacked bar with seven channels: Fragment KE, Prompt n KE, Prompt γ, β KE, β-delayed γ, Anti-νₑ, Other. Each channel's value is the sum of `initial_KE_MeV` over tracks matching `(creator_process, particle)` rules. The bar sums to the event's total tracked KE (~190-205 MeV per event for U-235 thermal fission). A second, lighter-shaded bar above shows the ENDF/B-VIII.0 reference partition (Fragment 169.1, prompt n 4.79, prompt γ 6.97, β 6.98, β-γ 6.30, ν̄ 8.75, total 202.9 MeV) so a reader can compare this event to the textbook mean. This is the canonical fission-energy-budget diagram from reactor-physics references (Lamarsh, ENDF, JEFF).
  - **Page 3 — Nuclear decay scheme.** Two side-by-side level-scheme staircases, one per fragment, drawn in NNDC/ENSDF style. The y-axis is cumulative Q-value released since the fragment's birth (MeV, increasing downward). Each ion in the chain is a horizontal level line labeled with the nuclide and cumulative Q to that level; each decay step is a downward arrow annotated with that step's Q-value and the kinetic energies of the emitted β / γ / α / anti-νₑ. Levels with no `RadioactiveDecay` children in the simulation window are labeled "(stable / no decay in sim window)". This complements page 2 by exposing chain topology and per-step Q-values that the partition aggregates over.
